...
public class WebSocketHandler {
        ...
        while (!cancellation.IsCancellationRequested && socket.State == WebSocketState.Open) {
            try {
                ...
                if (socket.State == WebSocketState.Open ||
                    ...
                    Console.WriteLine($"{DateTime.Now}: Client Disconnected Via Exception: " + index + " EX: " +
                                      ex.Message);
                    Console.ResetColor();
    ...
    private void ReceivedRequestAddUserToRoom(int index, string[] messageChunks, CommunicationTypeEnum com) {
        ...
        switch (addUserToServerRoomStatus) {
            ...
            case Room.RoomStatusCodes.RoomLocked:
                SendErrorMessage(index, com,
                    $"Room {userAddedToRoom.GetRoomName()} is locked and you can't join right now");
                break;
    ...
    private void SendUserListJsonPaginated(int index, CommunicationTypeEnum command, int sentNumber, int count, int c,
        List<User> list) {
        var send = new[]
        {
            $"{command}",
    ...
    private void SendRoomUserListJsonPaginated(Room room, int index, CommunicationTypeEnum command, int sentNumber,
        int count, int c, List<User> list) {
        var send = new[]
        {
            $"{command}",
            $"{sentNumber}",
    ...
    private void SendMessage(int index, string[] send) {
        ...
        if (logginEnabled) {
            ...
            Console.WriteLine(
                $"Sent To Client {index} {_userController.GetUserProfileFromSocketId(index)} Message: {message}");
            Console.ResetColor();
        }
    }
}